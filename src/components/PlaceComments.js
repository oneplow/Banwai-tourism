...
export default function PlaceComments({ placeId, initialComments = [], avgRating = 0 }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setComments([{ ...newComment, replies: [], isNew: true }, ...comments]);
        setStatus("success");
    ...
    <section className="mb-8">
        ...
        {comments.length > 0 && (
          <div className="flex items-center gap-2 bg-amber-50 px-3 py-1.5 rounded-full border border-amber-100">
            <span className="text-amber-500 text-sm font-bold">
              {avgRating > 0 ? avgRating.toFixed(1) : (comments.reduce((a, b) => a + b.rating, 0) / comments.length).toFixed(1)}
            </span>
            <StarRating rating={Math.round(avgRating) || Math.round(comments.reduce((a, b) => a + b.rating, 0) / comments.length) || 0} size="sm" />
      ...
      {comments.length === 0 ? (
        <div className="text-center py-10 bg-gray-50 rounded-2xl">
          ...
          <p className="text-gray-400 text-sm">ยังไม่มีความคิดเห็น เป็นคนแรกที่แสดงความคิดเห็นสถานที่นี้!</p>
        </div>
      ) : (
        <div className="space-y-4">
          {comments.map((item) => (
            <div
              key={item.comment_id}
              className={`bg-white rounded-xl p-4 border shadow-sm transition-all ${item.isNew ? "border-yellow-200 bg-yellow-50/10" : "border-gray-50"}`}
            >
